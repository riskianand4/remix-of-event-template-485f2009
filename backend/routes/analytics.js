...
router.get('/trends', auth, [
  ...
  try {
    ...
    console.log('Fetching stock movements for period:', { period, days, startDate });

    // Daily stock movements trend - Format for frontend consumption
    const movementTrends = await StockMovement.aggregate([
      {
        $match: {
          createdAt: { $gte: startDate }
    ...
    // Format stock movements for frontend chart
    ...
    res.json({
      ...
      data: {
        movementTrends,
        stockMovements,  // Add this for frontend chart
        stockValueTrend,
        period: {
          days,
          startDate,
