  ...
  async (req, res) => {
    try {
      ...
      const summary = {
        ...
        lowStockItems: report.filter((item) => item.stockStatus === "low_stock")
          .length,
        outOfStockItems: report.filter(
      ...
      if (req.query.format === "csv") {
        // Convert to CSV format - Fixed to remove price and stockValue
        const csvHeaders =
          "SKU,Name,Category,Unit,Current Stock,Minimum Stock,Maximum Stock,Stock Status,Status,Supplier\n";
        const csvRows = report
          .map(
            (item) =>
              `${item.sku},${item.name},${item.category},${item.unit},${item.currentStock},${item.minimumStock},${item.maximumStock},${item.stockStatus},${item.status},${item.supplier}`
          )
          .join("\n");
    ...
    try {
      ...
      const report = movements.map((movement) => ({
        ...
        product: {
          ...
          unit: movement.product?.unit || "N/A",
        },
        type: movement.type,
      ...
      if (req.query.format === "csv") {
        const csvHeaders =
          "Date,Product SKU,Product Name,Unit,Type,Quantity,Previous Stock,New Stock,Reason,Cost,Created By\n";
        const csvRows = report
          .map(
            (item) =>
              `${item.date.toISOString()},${item.product.sku},${
                item.product.name
              },${item.product.unit},${item.type},${item.quantity},${item.previousStock},${
                item.newStock
              },${item.reason},${item.cost},${item.createdBy}`
          )
          .join("\n");
  ...
  async (req, res) => {
    try {
      ...
      if (req.query.threshold) {
        stockFilter = {
          "stock.current": { $lte: parseInt(req.query.threshold) },
        };
      } else {
      ...
      const summary = {
        ...
        categoriesAffected: [...new Set(report.map((item) => item.category))]
          .length,
        suppliersToContact: [
          ...new Set(
            report
              .map((item) => item.supplier)
              .filter((s) => s !== "No supplier")
          ),
        ],
  ...
  async (req, res) => {
    try {
      ...
      if (req.query.assignedTo)
        filter["assignedTo.user"] = req.query.assignedTo;
      ...
      const summary = {
        ...
        totalDepreciation: report.reduce(
        ...
        maintenanceDue: report.filter(
          (item) =>
            item.maintenance.nextMaintenance &&
            new Date(item.maintenance.nextMaintenance) <=
      ...
      if (req.query.format === "csv") {
        const csvHeaders =
          "Asset Code,Name,Category,Status,Condition,Purchase Price,Current Value,Assigned To,Location,Purchase Date\n";
        const csvRows = report
          .map(
            (item) =>
              `${item.assetCode},${item.name},${item.category},${item.status},${
                item.condition
              },${item.purchasePrice},${item.currentValue},${
                item.assignedTo?.name || ""
              },${item.location.department || ""},${item.purchaseDate}`
          )
          .join("\n");
...
// @desc    Generate ABC analysis report (Fixed - no more price dependency)
...
router.get(
  ...
  [
    ...
    query("method")
      .optional()
      .isIn(["volume", "frequency", "cost"])
      .withMessage("Invalid method"),
  ],
  async (req, res) => {
    try {
      const period = req.query.period || "90d";
      const method = req.query.method || "volume"; // Default to volume-based analysis
      ...
      const productAnalysis = await StockMovement.aggregate([
        ...
        {
          $addFields: {
            // Choose analysis metric based on method
            analysisValue: {
              $switch: {
                branches: [
                  { case: { $eq: [method, "volume"] }, then: "$totalQuantity" },
                  { case: { $eq: [method, "frequency"] }, then: "$movementCount" },
                  { case: { $eq: [method, "cost"] }, then: "$totalValue" }
                ],
        ...
        { $sort: { analysisValue: -1 } },
      ]);

      // Calculate cumulative percentages for ABC classification
      const totalValue = productAnalysis.reduce(
        (sum, item) => sum + item.analysisValue,
        0
      );
      let cumulativeValue = 0;

      const abcClassified = productAnalysis.map((item) => {
        cumulativeValue += item.analysisValue;
        ...
        return {
          ...
          analysis: {
            ...
            totalCostValue: item.totalValue,
      ...
      const summary = {
        ...
        totalAnalysisValue: totalValue,
        method: method,
        classificationBreakdown: {
          A: abcClassified.filter(
            (item) => item.analysis.classification === "A"
          ).length,
          B: abcClassified.filter(
            (item) => item.analysis.classification === "B"
          ).length,
          C: abcClassified.filter(
            (item) => item.analysis.classification === "C"
          ).length,
        },
        valueByClass: {
          A: abcClassified
            .filter((item) => item.analysis.classification === "A")
            .reduce((sum, item) => sum + item.analysis.analysisValue, 0),
          B: abcClassified
