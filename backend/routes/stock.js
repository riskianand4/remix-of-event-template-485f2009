...
router.post('/movements', superAdminAuth, [
  ...
  body('movementDate').optional().isISO8601().withMessage('Invalid date format')
], async (req, res) => {
  try {
    const errors = validationResult(req);
    if (!errors.isEmpty()) {
    ...
    const { product: productId, type, quantity, reason, reference, cost, supplier, location, batchNumber, expiryDate, notes, movementDate } = req.body;
