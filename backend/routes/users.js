...
router.get(
  "/",
  superAdminAuth,
  activityLoggers.userAccess,
  [
    query("page").optional().isInt({ min: 1 }),
    query("limit").optional().isInt({ min: 1, max: 100 }),
    query("role").optional().isIn(["user", "admin", "super_admin", "teknisi"]),
...
router.post(
  ...
  [
    ...
    body("role")
      .optional()
      .isIn(["user", "cs", "super_admin", "teknisi"])
  ...
  async (req, res) => {
    try {
      ...
      try {
        ...
        // Don't fail user creation if email fails, but log itDashboard Teknisi
      }
...
router.put(
  ...
  async (req, res) => {
    try {
      ...
      if (
        req.body.role &&
        (req.body.role === "admin" || req.body.role === "super_admin") &&
        req.user.role !== "super_admin"
      ) {
        return res
          .status(403)
      ...
      if (user.role === "super_admin" && req.user.role !== "super_admin") {
        return res
          .status(403)
          .json({ error: "Cannot update super admin user" });
      }
...
router.patch("/:id/toggle-status", superAdminAuth, async (req, res) => {
  try {
    ...
    if (user.role === "super_admin" && req.user.role !== "super_admin") {
      return res
        .status(403)
        .json({ error: "Cannot modify super admin user status" });
    }
  ...
  async (req, res) => {
    try {
      ...
      if (user.role === "super_admin" && req.user.role !== "super_admin") {
        return res
          .status(403)
          .json({ error: "Cannot reset super admin password" });
      }
