...
import { MoreHorizontal, Search, Filter, Edit, Trash2, UserPlus, ExternalLink, RotateCcw, Package, TrendingUp, Download } from 'lucide-react';
...
export const AssetTable: React.FC<AssetTableProps> = ({
  ...
  const getConditionBadge = (condition: Asset['condition']) => {
    ...
    return <Badge variant="outline" className={variants[condition]}>
        {labels[condition]}
      </Badge>;
  };
  ...
  return <Card className="glass">
      <CardHeader>
        <div className="flex flex-col sm:flex-row gap-4">
          <div className="relative flex-1">
            ...
            <Input placeholder="Cari asset..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} className="pl-10" />
          </div>
      ...
      <CardContent>
        <div className="max-h-[calc(100vh)] overflow-x-auto overflow-y-auto">
          <div className="min-w-[900px]">
            <Table>
              ...
              <TableBody>
                {filteredAssets.length === 0 ? <TableRow>
                    <TableCell colSpan={10} className="text-center py-8 text-muted-foreground">
                      Tidak ada asset yang ditemukan
                    </TableCell>
                  </TableRow> : filteredAssets.map(asset => <TableRow key={asset.id} className="cursor-pointer hover:bg-muted/50 transition-colors h-[70px]" onClick={() => handleViewDetail(asset)}>
                          ...
                          <DropdownMenu>
                            ...
                            <DropdownMenuContent align="end" className="bg-background border-border shadow-lg z-50">
                              ...
                              {canEdit && <>
                                  ...
                                  {canAssignPIC && <DropdownMenuItem onClick={() => onAssignPIC?.(asset)}>
                                      <UserPlus className="mr-2 h-4 w-4" />
                                      Tugaskan PIC
                                    </DropdownMenuItem>}

                                  {asset.status === 'available' && asset.picId && <DropdownMenuItem onClick={() => onBorrow?.(asset)}>
                                      <ExternalLink className="mr-2 h-4 w-4" />
                                      Pinjam Asset
                                    </DropdownMenuItem>}

                                  {asset.status === 'borrowed' && <DropdownMenuItem onClick={() => onReturn?.(asset)}>
                                  ...
                                  {asset.status !== 'borrowed' && <>
                                      <DropdownMenuSeparator />
                                      <DropdownMenuItem onClick={() => setDeleteAssetId(asset.id)} className="text-destructive">
        ...
        <AlertDialog open={!!deleteAssetId} onOpenChange={() => setDeleteAssetId(null)}>
          <AlertDialogContent className="rounded-md max-w-xs md:max-w-xl sm:max-w-2xl ">
            <AlertDialogHeader>
        ...
        <Dialog open={detailDialogOpen} onOpenChange={setDetailDialogOpen}>
          <DialogContent className="sm:max-w-2xl mx-0 md:max-w-2xl max-w-[360px]">
            <DialogHeader>
            ...
            {selectedAsset && <div className="space-y-6">
                {/* Asset Overview */}
                ...
                <div className="border rounded-lg p-4 bg-muted/30">
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    ...
                    {selectedAsset.purchaseDate && <div className="space-y-1">
                        ...
                        <p className="text-sm bg-background rounded px-2 py-1 border">
                          {format(new Date(selectedAsset.purchaseDate), 'dd MMM yyyy', {
                      locale: id
                    })}
                        </p>
                      </div>}
                ...
                <div className="border rounded-lg p-4 bg-muted/30">
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    ...
                    <div className="space-y-1">
                      ...
                      <p className="text-sm bg-background rounded px-2 py-1 border">
                        {format(new Date(selectedAsset.createdAt), 'dd MMM yyyy HH:mm', {
                      locale: id
                    })}
                      </p>
                    </div>
                ...
                {/* Description */}
                {selectedAsset.description && <div className="border rounded-lg p-4 bg-muted/30">
                    <h4 className="text-sm font-semibold text-muted-foreground mb-3 flex items-center gap-2">
