...
import { Package, MapPin, Building2, DollarSign, Calendar, Edit, Save, X } from 'lucide-react';
...
const ProductDetailModal: React.FC<ProductDetailModalProps> = ({
  ...
  const currentStockStatus = product?.stockStatus || (product?.stock === 0 ? 'out_of_stock' : (product?.stock || 0) <= (product?.minStock || 0) ? 'low_stock' : 'in_stock');
  ...
  return <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent className="sm:max-w-3xl max-w-[360px] md:max-w-2xl mx-0  rounded-md">
        ...
        <div className="space-y-6">
          ...
          <div className="flex items-center justify-between p-4 bg-muted/30 rounded-lg border border-border">
            <div className="flex-1">
              {isEditing ? <div className="space-y-2">
                  ...
                  <Input id="name" value={editedProduct.name} onChange={e => setEditedProduct(prev => ({
                ...prev,
                name: e.target.value
              }))} />
                </div> : <>
              ...
              <div className="space-y-4">
                {isEditing ? <>
                    <div className="space-y-2">
                      ...
                      <Input id="stock" type="number" value={editedProduct.stock} onChange={e => setEditedProduct(prev => ({
                    ...prev,
                    ...
                    <div className="space-y-2">
                      ...
                      <Input id="minStock" type="number" value={editedProduct.minStock} onChange={e => setEditedProduct(prev => ({
                    ...prev,
              ...
              <div className="space-y-4">
                {isEditing ? <div className="space-y-2">
                   ...
                   <div className="space-y-2">
                     ...
                     <p className="text-lg font-bold text-primary bg-background rounded px-3 py-2 border border-border">{product?.unit || 'pcs'}</p>
                   </div>
                ...
                {isEditing ? <div className="space-y-2">
                    ...
                    <Select value={editedProduct.location || ''} onValueChange={value => setEditedProduct(prev => ({
                  ...prev,
                  location: value
                }))}>
                ...
                {product?.supplier && <div className="space-y-2">
          ...
          {/* Description */}
          {(product?.description || isEditing) && <div className="border border-border rounded-lg p-4">
              <h3 className="text-sm font-semibold text-foreground mb-3">Deskripsi</h3>
              {isEditing ? <Textarea value={editedProduct.description || ''} onChange={e => setEditedProduct(prev => ({
            ...prev,
            description: e.target.value
          }))} placeholder="Masukkan deskripsi produk..." rows={3} className="bg-background border-border" /> : <p className="text-sm text-muted-foreground bg-background rounded px-3 py-2 border border-border">{product?.description || 'Tidak ada deskripsi'}</p>}
            </div>}
