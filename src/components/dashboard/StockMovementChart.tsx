...
const StockMovementChart: React.FC<StockMovementChartProps> = ({
  ...
  const [chartType, setChartType] = useState<'area'>('area');
  const months = [{
    value: '0',
    label: 'Januari'
  }, {
  ...
  const fetchData = async () => {
    try {
      ...
      if (!movementData || movementData.length === 0) {
        ...
        const sampleData: StockMovementFlow[] = [];
        ...
        sampleData.forEach(item => {
          item.netFlow = item.stockIn - item.stockOut;
        });
        
        setData(sampleData);
      } else {
        // Filter and process real data by selected month/year
        const filteredData = movementData.filter(item => {
          ...
          ...item,
          // Ensure absolute values for stock out for display
          stockOut: Math.abs(item.stockOut || 0)
        }));
        
        setData(filteredData);
      }
    } catch (err) {
      setError('Gagal memuat data pergerakan stock. Menampilkan data contoh.');
      
      // Generate fallback sample data even on error
      const daysInMonth = new Date(parseInt(selectedYear), parseInt(selectedMonth) + 1, 0).getDate();
      const fallbackData: StockMovementFlow[] = [];
      ...
      <AreaChart data={data}>
        ...
        <Tooltip 
          ...
          formatter={(value: any, name: string) => [
            `${Math.abs(value)} unit`, 
            name === 'stockIn' ? 'Stock Masuk' : 
            name === 'stockOut' ? 'Stock Keluar' : 'Net Flow'
          ]} 
        ...
        {data.length > 0 ? (
          ...
          <div className="flex flex-col items-center justify-center h-48 sm:h-64 text-center">
            ...
            <p className="text-xs text-muted-foreground mt-2">
              Data akan muncul setelah ada transaksi stok masuk/keluar
            </p>
          </div>
        )}
      </CardContent>
