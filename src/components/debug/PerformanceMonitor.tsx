    ...
    if ('PerformanceObserver' in window) {
      const observer = new PerformanceObserver((list) => {
        ...
        entries.forEach((entry) => {
          // Check for API calls using environment config URL
          const apiUrlPattern = ENV.API_BASE_URL.replace(/https?:\/\//, '');
          if (entry.name.includes('/api/') || entry.name.includes(apiUrlPattern)) {
            try {
