...
const InventoryOverview = ({
  ...
  const stats = {
    ...
    totalValue: 0 // Removed price calculation
  };
  if (isLoading) {
  ...
  return <div className="space-y-4 sm:space-y-6 p-0 sm:p-0">
      ...
      >
        <div>
          ...
          <p className="text-sm text-muted-foreground">Kelola dan pantau inventori perusahaan</p>
        </div>
        <Button onClick={handleRefresh} variant="outline" size="sm" disabled={isLoading} className="glass">
      ...
      {/* Stock Alert Banner */}
      <StockAlertBanner products={inventoryItems} onViewProduct={productId => {
      ...
      duration: 0.3
        ...
        <Card className="bg-accent/10 border-accent/20">
          <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2 p-3 sm:p-4">
            <CardTitle className="text-xs sm:text-sm font-medium">Total Unit Inventori</CardTitle>
            <TrendingUp className="h-3 w-3 sm:h-4 sm:w-4 text-info" />
          </CardHeader>
          <CardContent className="pt-0 p-3 sm:p-4">
            <div className="text-lg sm:text-xl lg:text-2xl font-bold text-info">
              {(stats.totalValue / 1000).toFixed(0)}K Unit
            </div>
            <p className="text-xs sm:text-sm text-muted-foreground">Total unit stok</p>
          </CardContent>
        </Card>
      </motion.div>
                    ...
                    {filteredItems.map(item => <TableRow key={item.id} className="cursor-pointer hover:bg-muted/50 transition-colors" onClick={() => handleViewDetail(item)}>
                        ...
                        <TableCell className="w-[100px] sm:w-[120px] p-2 sm:p-4">
                          <div className="w-16 sm:w-24">
                            ...
                            <div className="text-xs text-muted-foreground mt-1" title={item.maxStock && item.maxStock > 0 ? `${item.stock}/${item.maxStock} (${getStockPercentage(item)}%)` : item.minStock > 0 ? `Stock: ${item.stock}, Min: ${item.minStock} (${getStockPercentage(item)}%)` : `Stock: ${item.stock} (${getStockPercentage(item)}%)`}>
      ...
      <Dialog open={detailDialogOpen} onOpenChange={setDetailDialogOpen}>
        <DialogContent className="rounded-md max-w-[360px] mx-0  md:max-w-xl sm:max-w-2xl  max-h-[90vh] overflow-y-auto">
                ...
                <div className="space-y-2">
                  ...
                  <div className="bg-background rounded p-3 border border-border">
                    ...
                    <p className="text-xs text-muted-foreground">
                      {getStockPercentage(selectedItem)}% - {selectedItem.maxStock && selectedItem.maxStock > 0 ? `${selectedItem.stock}/${selectedItem.maxStock} (berdasarkan maxStock)` : selectedItem.minStock > 0 ? `Relatif terhadap minStock (${selectedItem.minStock})` : 'Berdasarkan estimasi kapasitas'}
                    </p>
                  </div>
                </div>
              </div>
