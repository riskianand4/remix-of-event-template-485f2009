...
import { Package, BarChart3, Plus, Wifi, Home, TrendingUp, HardDrive, Users, Shield, Settings, Database, AlertTriangle, FileText, Monitor, Key, Bell, Download, User, Wrench } from 'lucide-react';
...
const additionalApps = [{
  ...
  icon: AlertTriangle,
  roles: ['user', 'admin', 'superadmin'],
  description: "Sistem tiket gangguan",
  badge: "New"
}];
...
export function AppSidebar() {
  ...
  return <>
      <Sidebar className={`${collapsed ? 'w-16' : 'w-60'}`}>
        ...
        <SidebarContent className="p-4 px-[1px]">
          {/* Main Navigation */}
          <SidebarGroup>
                      ...
                      delay: index * 0.05
                          ...
                          <NavLink to={item.url} className={`flex items-center space-x-3 p-3 rounded-lg transition-all duration-200 group ${isActive ? 'bg-sidebar-accent text-sidebar-accent-foreground shadow-medium' : 'text-sidebar-foreground/70 hover:text-sidebar-foreground hover:bg-sidebar-accent/50'}`}>
                            <Icon className={`w-5 h-5 ${isActive ? 'text-sidebar-accent-foreground' : 'group-hover:text-sidebar-primary'}`} />
                             {!collapsed && <span className="font-medium text-xs md:text-xs">{item.title}</span>}
                              ...
                              <Icon className={`w-5 h-5 ${isActive ? 'text-sidebar-accent-foreground' : 'group-hover:text-sidebar-primary'}`} />
                               {!collapsed && (
                                 <div className="flex items-center justify-between w-full">
                                   <span className="font-medium text-xs md:text-xs">{item.title}</span>
                                   {item.badge && (
