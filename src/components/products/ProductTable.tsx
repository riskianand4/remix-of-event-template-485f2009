...
const ProductTable = ({
  ...
  return <>
      <div className="mobile-table-container">
        <Table className="mobile-table-wrapper">
          ...
          <TableBody>
            ...
            return <TableRow key={product.id} className={`cursor-pointer hover:bg-muted/50 transition-colors h-[70px] ${isSelected ? "bg-primary/5" : ""}`} onClick={() => handleViewProduct(product)}>
                    {isAdmin && <TableCell className="w-[50px]" onClick={e => e.stopPropagation()}>
                        <Checkbox checked={isSelected} onCheckedChange={checked => handleSelectProduct(product.id, checked as boolean)} />
                      </TableCell>}
                    ...
                    <TableCell className="w-[200px]">
                      <div className="flex items-center gap-3">
                        ...
                        <div className="min-w-0">
                          ...
                          {product.description && <p className="text-xs text-muted-foreground truncate" title={product.description}>
                              {product.description}
                            </p>}
                        </div>
                      </div>
                    ...
                    <TableCell className="w-[100px]">
                      <div className="text-xs truncate" title={displayText(product.category)}>
                    ...
                    <TableCell className="w-[100px]">
                      <Badge variant="outline" className={`text-xs ${stockStatus === "in_stock" ? "bg-success/10 text-success border-success/20" : stockStatus === "low_stock" ? "bg-warning/10 text-warning border-warning/20" : stockStatus === "out_of_stock" ? "bg-destructive/10 text-destructive border-destructive/20" : "bg-muted text-muted-foreground border-muted/50"}`}>
                        {getStockStatusLabel(stockStatus)}
                      </Badge>
                    </TableCell>

                    <TableCell className="w-[120px]">
                      <div className="text-xs text-muted-foreground truncate" title={displayText(product.location)}>
                        {displayText(product.location)}
                      </div>
                    </TableCell>

                    <TableCell className="w-[120px]">
                      <div className="text-xs text-muted-foreground truncate" title={displayText(product.supplier)}>
      ...
      <Dialog open={deleteDialogOpen} onOpenChange={setDeleteDialogOpen}>
        <DialogContent className="rounded-md max-w-xs md:max-w-xl sm:max-w-2xl ">
          <DialogHeader>
          ...
          {productDetail && <div className="space-y-4">
              <div className="grid grid-cols-2 gap-4">
                ...
                <div>
                  ...
                  <p className="text-sm">{productDetail.sku}</p>
                </div>
              </div>
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                <div>
                  ...
                  <Badge variant="outline" className={`text-xs ${getProductStockStatus(productDetail) === "in_stock" ? "bg-success/10 text-success border-success/20" : getProductStockStatus(productDetail) === "low_stock" ? "bg-warning/10 text-warning border-warning/20" : getProductStockStatus(productDetail) === "out_of_stock" ? "bg-destructive/10 text-destructive border-destructive/20" : "bg-muted text-muted-foreground border-muted/50"}`}>
