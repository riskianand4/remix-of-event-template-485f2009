...
export type SortOption = "name" | "unit" | "stock" | "category" | "updated";
const ProductsManager = () => {
  ...
  const stats = useMemo(() => {
    ...
    return {
      ...
      totalValue: products.reduce((sum, p) => sum + (p.unit === 'pcs' ? p.stock : 0), 0),
    };
  }, [products]);
  const statItems = [
    {
  ...
  return (
    <div className="min-h-screen  sm:p-0 lg:p-0 ">
      <motion.div
        initial={{
          opacity: 0,
          y: 20,
        ...
        className=" mx-auto space-y-3 sm:space-y-4 lg:space-y-6"
      >
        {/* Header */}
        <div className="flex flex-col space-y-3 sm:flex-row sm:items-center sm:justify-between sm:space-y-0">
          <div>
            <h1 className="text-xl sm:text-2xl font-bold">Manajemen Produk</h1>
            <p className="text-sm text-muted-foreground">Kelola dan pantau semua produk</p>
          </div>
          {/* Action Buttons */}
