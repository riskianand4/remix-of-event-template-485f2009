...
import { BarChart3, FileText, Plus, Users, Database, PieChart } from 'lucide-react';
...
const navigation = [{
  ...
  href: '/psb-report',
  icon: BarChart3,
  description: 'Overview dan monitoring real-time'
}, {
  title: 'Input Order',
  href: '/psb-report/input',
  icon: Plus,
  description: 'Order masuk - data pelanggan baru'
}, {
  title: 'Distribution',
  href: '/psb-report/distribution',
  icon: Users,
  description: 'Assign teknisi ke order'
}, {
  title: 'Activation',
  href: '/psb-report/activation',
  icon: PieChart,
  description: 'Setting ONT & aktivasi layanan'
}, {
  title: 'Data Management',
  href: '/psb-report/data',
  icon: Database,
  description: 'Kelola data pelanggan & order'
}, {
  title: 'Reports',
  href: '/psb-report/reports',
  icon: FileText,
  description: 'Laporan & analytics performa'
}];
  ...
  return (
    <Sidebar className={`${collapsed ? 'w-16' : 'w-60'} hidden md:flex border-r transition-all duration-300`}>
      <SidebarHeader className="border-b py-3 px-6">
        <motion.div className="flex items-center gap-3" initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.3 }}>
          ...
          {!collapsed && (
            <div>
              ...
              <p className="text-[10px] text-muted-foreground">PSB & Gangguan Management</p>
            </div>
          )}
        </motion.div>
      </SidebarHeader>
                      ...
                      <SidebarMenuButton asChild>
                        <NavLink to={item.href} end={item.href === '/psb-report'} className={`flex items-center gap-3 px-3 py-5 rounded-lg transition-all duration-200 group relative overflow-hidden ${itemIsActive ? 'bg-gradient-to-r from-primary/5 to-primary/10 text-foreground shadow-md' : 'text-muted-foreground hover:text-foreground hover:bg-gradient-to-r hover:from-primary/5 hover:to-primary/10 hover:shadow-md'}`}>
                          ...
                          {itemIsActive && !collapsed && (
                            <motion.div className="w-2 h-2 bg-primary rounded-full" layoutId="activeIndicator" initial={{ scale: 0 }} animate={{ scale: 1 }} transition={{ type: "spring", stiffness: 500, damping: 30 }} />
                          )}
