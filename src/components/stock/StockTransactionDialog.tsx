...
const StockTransactionDialog: React.FC<StockTransactionDialogProps> = ({
  ...
  const { createMovement, loading } = useEnhancedStockManager();
  ...
  const onSubmit = async (data: StockTransactionFormData) => {
    try {
      const movement: Omit<StockMovement, 'id'> = {
        ...
        // location: data.location, // Removed as not in StockMovement interface
        // warehouse: data.warehouse || '', // Removed as not in interface
        // timestamp: new Date(), // Removed as not in interface
        // userId: 'current-user', // Removed as not in interface
        // userName: 'Current User', // Removed as not in interface
        cost: data.cost,
        // unitPrice: data.unitPrice, // Removed as not in interface
        supplier: (data.supplier || '') as any,
        // notes: data.notes || '', // Removed as not in interface
        status: 'completed',
        createdAt: new Date(),
      };
