...
export const AddUserDialog: React.FC<AddUserDialogProps> = ({
  ...
  const [formData, setFormData] = useState({
    ...
    email: '',
    phone: '',
    role: 'user' as 'user' | 'superadmin' | 'teknisi' | 'cs',
    department: '',
    position: '',
    password: ''
  });
  ...
  const filteredDepartments = departments.filter(dept => dept.toLowerCase().includes(deptSearch.toLowerCase()));
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const errorMessage = error instanceof ApiClientError ? error.message : error instanceof Error ? error.message : 'Gagal membuat user';
  ...
  return <Dialog open={isOpen} onOpenChange={setIsOpen}>
      ...
      <DialogContent className="rounded-md max-w-[360px] md:max-w-2xl sm:max-w-3xl  mx-0  max-h-[90vh] overflow-y-auto">
        ...
        <form onSubmit={handleSubmit}>
          <div className="grid gap-3 sm:gap-4 py-3 sm:py-4">
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 sm:gap-4">
              ...
              }} placeholder="Masukkan nama lengkap" className={`text-sm ${formErrors.name ? 'border-destructive' : ''}`} required />
              ...
              }} placeholder="user@telnet.co.id" className={`text-sm ${formErrors.email ? 'border-destructive' : ''}`} required />
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 sm:gap-4">
              ...
              }} placeholder="0812-3456-7890" className={`text-sm ${formErrors.phone ? 'border-destructive' : ''}`} />
              ...
              <div className="grid gap-2">
                ...
                ...prev,
                role: value as 'user' | 'superadmin' | 'teknisi' | 'cs'
              }))}>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 sm:gap-4">
              <div className="grid gap-2">
                ...
                <Popover open={deptOpen} onOpenChange={setDeptOpen}>
                  <PopoverTrigger asChild>
                    <Button variant="outline" role="combobox" aria-expanded={deptOpen} className="justify-between text-sm h-9">
                  ...
                  <PopoverContent className="w-full sm:w-[300px] p-0 z-50">
                    <Command>
                      <CommandInput placeholder="Cari atau ketik department..." value={deptSearch} onValueChange={setDeptSearch} className="text-sm" />
                      <CommandEmpty>
                        {deptSearch && <div className="p-2">
                            <Button variant="ghost" className="w-full justify-start text-xs sm:text-sm" onClick={() => handleDepartmentSelect('__new__')}>
                      ...
                      <CommandGroup>
                        {filteredDepartments.map((dept, index) => <CommandItem key={index} value={dept} onSelect={() => handleDepartmentSelect(dept)} className="text-sm">
                            <Check className={cn("mr-2 h-3 w-3 sm:h-4 sm:w-4", formData.department === dept ? "opacity-100" : "opacity-0")} />
              ...
              <div className="grid gap-2">
                ...
                <Input id="position" value={formData.position} onChange={e => setFormData(prev => ({
                ...prev,
                position: e.target.value
              }))} placeholder="Jabatan/posisi" className="text-sm" />
              </div>
            </div>
            <div className="grid gap-2">
              ...
              <div className="space-y-2 sm:space-y-3">
                <div className="flex gap-2">
                  <div className="relative flex-1">
                    <Input id="password" type={showPassword ? "text" : "password"} value={formData.password} onChange={e => {
                    setFormData(prev => ({
                      ...prev,
                      password: e.target.value
                    }));
                    if (formErrors.password) setFormErrors(prev => ({
                      ...prev,
                      password: ''
                    }));
                  }} placeholder="Masukkan password sementara" className={`pr-10 text-sm ${formErrors.password ? 'border-destructive' : ''}`} required />
                    <Button type="button" variant="ghost" size="sm" className="absolute right-1 top-1/2 transform -translate-y-1/2 w-6 h-6 p-0" onClick={() => setShowPassword(!showPassword)}>
                      {showPassword ? <EyeOff className="w-3 h-3 sm:w-4 sm:h-4" /> : <Eye className="w-3 h-3 sm:w-4 sm:h-4" />}
                    </Button>
                  </div>
                  <Button type="button" variant="outline" size="sm" onClick={generatePassword} className="px-2 sm:px-3">
          ...
          <DialogFooter className="flex flex-col-reverse sm:flex-row gap-2 sm:gap-2 pt-3 sm:pt-4">
            ...
            <Button type="submit" disabled={isLoading} className="w-full sm:w-auto text-xs sm:text-sm" size="sm">
      ...
      {pendingUserData && <EmailVerificationDialog open={showVerification} onOpenChange={setShowVerification} email={pendingUserData.email} type="user_creation" userName={pendingUserData.name} onVerificationComplete={handleVerificationComplete} />}
    </Dialog>;
};