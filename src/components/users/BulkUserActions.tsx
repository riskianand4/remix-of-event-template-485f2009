...
import { Users, UserCheck, UserX, Trash2, Shield, Download, Upload, AlertTriangle, Loader2 } from 'lucide-react';
import { toast } from 'sonner';
interface BulkUserActionsProps {
  selectedUsers: User[];
  onClearSelection: () => void;
  onBulkStatusUpdate: (userIds: string[], status: 'active' | 'inactive' | 'suspended') => Promise<boolean>;
  onBulkRoleUpdate: (userIds: string[], role: 'user' | 'superadmin' | 'teknisi') => Promise<boolean>;
...
export const BulkUserActions: React.FC<BulkUserActionsProps> = ({
  ...
  const handleBulkRoleChange = (role: 'user' | 'superadmin' | 'teknisi') => {
    const userIds = selectedUsers.map(u => u.id);
    handleBulkAction({
      type: 'role',
      value: role,
  ...
  return <>
        ...
        <CardContent>
          <div className="space-y-4">
            ...
            <div className="flex flex-wrap gap-1 max-h-20 overflow-y-auto">
              {selectedUsers.map(user => <Badge key={user.id} variant="secondary" className="text-xs">
                  {user.name}
                </Badge>)}
            </div>
            ...
            <div className="space-y-2">
              ...
              <div className="flex flex-wrap gap-2">
                <Button size="sm" variant="outline" onClick={() => handleBulkStatusChange('active')} className="text-success border-success hover:bg-success/10">
                  <UserCheck className="w-4 h-4 mr-1" />
                  Activate
                </Button>
                <Button size="sm" variant="outline" onClick={() => handleBulkStatusChange('inactive')} className="text-muted-foreground">
                  <UserX className="w-4 h-4 mr-1" />
                  Deactivate
                </Button>
                <Button size="sm" variant="outline" onClick={() => handleBulkStatusChange('suspended')} className="text-destructive border-destructive hover:bg-destructive/10">
                  <UserX className="w-4 h-4 mr-1" />
                  Suspend
                </Button>
              </div>
            ...
            <div className="space-y-2">
              ...
              <div className="flex flex-wrap gap-2">
                ...
                <Button size="sm" variant="outline" onClick={() => handleBulkRoleChange('superadmin')} className="text-warning border-warning hover:bg-warning/10">
                  <Shield className="w-4 h-4 mr-1" />
                  Super Admin
                </Button>
                <Button size="sm" variant="outline" onClick={() => handleBulkRoleChange('teknisi')} className="text-primary border-primary hover:bg-primary/10">
                  <Shield className="w-4 h-4 mr-1" />
                  Teknisi
                </Button>
              </div>
      ...
      <Dialog open={isConfirmDialogOpen} onOpenChange={setIsConfirmDialogOpen}>
        <DialogContent className="bg-background">
          ...
          {confirmAction?.type === 'delete' && <div className="bg-destructive/10 border border-destructive/20 rounded p-3">
          ...
          <DialogFooter>
            ...
            <Button variant={confirmAction?.type === 'delete' ? 'destructive' : 'default'} onClick={executeAction} disabled={isProcessing}>
