...
import { Eye, Edit, Trash2, Lock, Unlock, MoreHorizontal, Activity, Mail, AlertTriangle } from 'lucide-react';
        ...
        <TableBody>
          {users.map((user) => {
            ...
            return (
              <TableRow 
                key={user.id} 
                className={`hover:bg-muted/50 h-12 ${isUnverified ? 'bg-destructive/10 border-l-4 border-l-destructive' : ''}`}
              >
                <TableCell className="py-2">
                  <div className="flex items-center space-x-2">
                    <div className="relative">
                      ...
                      {isUnverified && (
                        <AlertTriangle className="absolute -top-1 -right-1 w-3 h-3 text-destructive bg-background rounded-full" />
                      )}
                    </div>
                    <div className="min-w-0 max-w-36">
                      <div className="flex items-center gap-1">
                        <p className={`font-medium text-sm truncate ${isUnverified ? 'text-destructive' : 'text-foreground'}`}>
                          {user.name}
                        </p>
                        {isUnverified && (
                          <button
                ...
                <TableCell className="py-2">
                  <div className="flex items-center gap-1">
                    <span className={`text-sm truncate block max-w-40 ${isUnverified ? 'text-destructive' : ''}`}>
                      {user.email}
                    </span>
                    {isUnverified && (
