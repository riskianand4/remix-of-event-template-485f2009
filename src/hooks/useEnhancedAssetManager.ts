...
export const useEnhancedAssetManager = () => {
  ...
  const addAsset = useCallback(
    async (assetData: Omit<Asset, "id" | "createdAt" | "updatedAt">) => {
      ...
      try {
        ...
        if (isConfigured && isOnline && apiService) {
          ...
          const backendData = {
            name: assetData.name,
            code: assetData.code, // Map code to code for backend
            category: assetData.category,
            description: assetData.description,
            purchasePrice: assetData.purchasePrice,
            purchaseDate: assetData.purchaseDate
              ? new Date(assetData.purchaseDate).toISOString()
              : null,
          ...
          if (response.success && response.data) {
            ...
            const newAsset: Asset = {
              ...
              location:
                typeof backendAsset.location === "string"
                  ? backendAsset.location
                  : `${backendAsset.location?.department || ""} ${
                      backendAsset.location?.room || ""
                    } ${backendAsset.location?.building || ""}`.trim(),
            ...
            toast.success(
              `Asset Added: ${newAsset.name} has been added successfully`
            );
          ...
          toast.success(
            `Asset Added Locally: ${newAsset.name} has been saved locally`
          );
  ...
  const borrowAsset = useCallback(
    async (borrowRequest: AssetBorrowRequest) => {
      if (!user || user.role !== "superadmin") {
