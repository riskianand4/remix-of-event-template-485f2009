...
export const useOptimizedAlerts = () => {
  ...
  const fetchBackendAlerts = useCallback(async () => {
    ...
    try {
      ...
      if (response.success && response.data) {
        const backendAlerts = response.data.map((alert: any) => ({
          id: alert.id || alert._id,
          productId: alert.product?.id || alert.product?._id || '',
          productName: alert.productName || alert.product?.name || alert.title,
          ...
          thresholdId: `backend-${alert.type}`,
        ...
        setState(prev => ({ ...prev, backendAlerts }));
      }
    } catch (error) {
      logger.error('Failed to fetch backend alerts:', error);
    }
  }, [isAuthenticated, user, logger]);

  // Fetch backend alerts on mount and periodically
  useEffect(() => {
    fetchBackendAlerts();
    const interval = setInterval(fetchBackendAlerts, 5 * 60 * 1000); // Every 5 minutes
  ...
  // Combine local and backend alerts
  const allAlerts = [...state.alerts, ...state.backendAlerts];
  
  return {
    alerts: allAlerts,
    unacknowledgedAlerts: allAlerts.filter(alert => !alert.acknowledged),
    criticalAlerts: allAlerts.filter(alert => alert.severity === 'CRITICAL' && !alert.acknowledged),
    isGenerating: state.isGenerating,
    error: state.error,
    generateAlerts,
    acknowledgeAlert,
    refreshBackendAlerts: fetchBackendAlerts,
    alertStats: {
      total: allAlerts.length,
      unacknowledged: allAlerts.filter(alert => !alert.acknowledged).length,
      critical: allAlerts.filter(alert => alert.severity === 'CRITICAL' && !alert.acknowledged).length,
    }
  };
};
