...
export const usePSBData = () => {
    ...
    async (params?: any, retryOnFailure = true) => {
      ...
      while (currentAttempt < maxRetries) {
        try {
          ...
          console.log("chek order", response);
          ...
          if (success || Array.isArray(ordersData)) {
            ...
            setLastUpdateTime(Date.now()); // Update last refresh time
  ...
  const updateTechnicianStatus = useCallback(
    async (
      id: string,
      ...
      try {
        setLoading(true);
        console.log(
          "[PSB Hook] Updating technician status:",
          id,
          status,
          reason
        );

        const response = await psbApi.updateTechnicianStatus(
          id,
          status,
          reason
        );
        toast.success("Status teknisi berhasil diperbarui");
        await fetchOrders(undefined, false);
        return response;
      } catch (error) {
        handleError(error, "updateTechnicianStatus");
        toast.error("Gagal mengupdate status teknisi");
        throw error;
      } finally {
        setLoading(false);
      }
    ...
    async (id: string, orderData: any) => {
      try {
        ...
        if (response.success) {
          ...
          setLastUpdateTime(Date.now()); // Update timestamp after successful update
          // Refresh orders list
          await fetchOrders(undefined, false); // Don't retry on refresh
          return response;
        } else {
  ...
  // Fetch all orders for Distribution Center (no pagination limit)
  const fetchAllOrders = useCallback(async () => {
    console.log("[PSB Hook] Fetching all orders for Distribution Center");
    return await fetchOrders({ limit: 1000, page: 1 }, false);
  }, [fetchOrders]);
