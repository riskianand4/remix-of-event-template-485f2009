...
import { Search, Plus, Download, Upload, AlertTriangle, Bell, BellOff, Eye, Edit, Trash2, Filter, Settings, CheckCircle, Clock } from "lucide-react";
...
export default function AlertsPage() {
  ...
  const filteredAlerts = allAlerts.filter(alert => {
    const matchesSearch = alert.title?.toLowerCase().includes(searchTerm.toLowerCase()) || alert.message.toLowerCase().includes(searchTerm.toLowerCase());
  ...
  return <ErrorBoundary>
      <MainLayout>
        <div className=" sm:p-3 lg:p-0 space-y-4 sm:space-y-6">
          {/* Header */}
          <motion.div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3 sm:gap-4" initial={{
          opacity: 0,
          y: -20
        }} animate={{
          opacity: 1,
          ...
          duration: 0.3
            ...
            <div className="flex gap-2 w-full sm:w-auto">
              {user?.role === "superadmin" && <Dialog open={isCreateDialogOpen} onOpenChange={setIsCreateDialogOpen}>
                  ...
                  <DialogContent className="mx-0 max-h-[90vh] overflow-y-auto rounded-md max-w-[360px] md:max-w-lg sm:max-w-xl ">
                    <DialogHeader>
                    ...
                    <form onSubmit={handleCreateAlert}>
                      <div className="grid gap-4 py-4">
                        ...
                        productId: ""
                          ...
                          {["LOW_STOCK", "OUT_OF_STOCK", "OVERSTOCK", "EXPIRING"].includes(formData.type) && <p className="text-xs text-muted-foreground">
                              Untuk tipe stok, pilih produk di bawah.
                            </p>}
                        </div>
                        {/* Product Selection for Stock Alerts */}
                        {["LOW_STOCK", "OUT_OF_STOCK", "OVERSTOCK", "EXPIRING"].includes(formData.type) && <div className="grid gap-2">
                            <Label htmlFor="productId" className="text-sm">
                              Produk *
                            </Label>
                            <Select value={formData.productId} onValueChange={value => setFormData({
                        ...formData,
                        productId: value
                        ...
                        <div className="grid gap-2">
                          ...
                          <Select value={formData.severity} onValueChange={value => setFormData({
                        ...formData,
                        severity: value
                      })}>
                            <SelectTrigger className="h-10">
                        ...
                        <div className="grid gap-2">
                          ...
                          <Input id="title" placeholder="Masukkan judul alert" value={formData.title} onChange={e => setFormData({
                        ...formData,
                        ...
                        <div className="grid gap-2">
                          ...
                          <Textarea id="message" placeholder="Deskripsi detail alert..." value={formData.message} onChange={e => setFormData({
                        ...formData,
                        message: e.target.value
                      })} required className="min-h-[80px]" />
                        </div>
                      </div>
                      <div className="flex flex-col sm:flex-row justify-end gap-2">
                        <Button type="button" variant="outline" onClick={() => setIsCreateDialogOpen(false)} disabled={isSubmitting} className="w-full sm:w-auto">
                          Batal
                        </Button>
          ...
          {/* Overview Cards */}
          <motion.div className="grid grid-cols-2 sm:grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-4 lg:gap-6" initial={{
          opacity: 0,
          y: 20
        }} animate={{
          opacity: 1,
          ...
          delay: 0.2
            ...
            <div className="relative w-full">
              ...
              <Input placeholder="Cari alert berdasarkan judul atau pesan..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} className="pl-10 bg-muted/50 h-10 sm:h-12" />
            </div>
                ...
                <Card>
                  <CardHeader className="p-3 sm:p-4 lg:p-6">
                    ...
                    <CardDescription className="text-xs sm:text-sm">
                      ...
                      {filteredAlerts.length !== totalAlerts && <span className="ml-2 text-primary">
                  ...
                  <CardContent className="p-0">
                    <div className="overflow-x-auto">
                      <Table className="min-w-[800px]">
                        ...
                        <TableBody>
                          {filteredAlerts.map(alert => <TableRow key={alert.id} className={!alert.isRead ? "bg-muted/30" : ""}>
                              <TableCell className="p-2 sm:p-4">
                                <div className="flex items-center gap-1 sm:gap-2">
                                  {!alert.isRead && <div className="w-2 h-2 bg-primary rounded-full flex-shrink-0"></div>}
                                  {alert.isResolved ? <CheckCircle className="w-3 h-3 sm:w-4 sm:h-4 text-success flex-shrink-0" /> : <Clock className="w-3 h-3 sm:w-4 sm:h-4 text-warning flex-shrink-0" />}
                                </div>
                              </TableCell>
                              <TableCell className="p-2 sm:p-4">
                              ...
                              <TableCell className="font-medium p-2 sm:p-4">
                                <div className="text-xs sm:text-sm truncate" title={alert.title || `${alert.type} Alert`}>
                                  {alert.title || `${alert.type} Alert`}
                                </div>
                              </TableCell>
                              <TableCell className="p-2 sm:p-4">
                                <div className="text-xs sm:text-sm truncate" title={alert.message}>
                                  {alert.message}
                                </div>
                              </TableCell>
                              <TableCell className="text-xs sm:text-sm text-muted-foreground p-2 sm:p-4">
                                <div className="truncate">
                                  {safeLocaleDateString(alert.timestamp, "id-ID", undefined, "Unknown date")}
                                </div>
                              </TableCell>
                              <TableCell className="p-2 sm:p-4">
                                ...
                                toast({
                                  ...
                                  {!alert.isRead && <Button variant="ghost" size="sm" onClick={() => handleAcknowledgeAlert(alert.id)} className="h-8 w-8 p-0">
                                      <CheckCircle className="w-3 h-3 sm:w-4 sm:h-4" />
                                    </Button>}
                                   {user?.role === "superadmin" && !alert.isResolved && <Button variant="ghost" size="sm" onClick={() => {
