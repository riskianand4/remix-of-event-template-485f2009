...
import { Eye, EyeOff, Plus, Copy, Trash2, RotateCcw, Activity, AlertTriangle, Check, Key, BookOpen, BarChart3, TestTube, Download, Calendar, MoreHorizontal } from "lucide-react";
import { Table, TableBody, TableCell, TableHead, TableHeader, TableRow } from "@/components/ui/table";
import { DropdownMenu, DropdownMenuContent, DropdownMenuItem, DropdownMenuTrigger } from "@/components/ui/dropdown-menu";
import { apiKeyService, type ApiKeyData, type CreateApiKeyRequest } from "@/services/apiKeyService";
...
const ApiKeyCreatedDialog: React.FC<ApiKeyCreatedDialogProps> = ({
  ...
  const downloadApiKey = () => {
    ...
    link.href = url;
    link.download = `${keyName.replace(/\s+/g, "_").toLowerCase()}_api_key.json`;
  ...
  return <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="w-full max-w-xs sm:max-w-md lg:max-w-[500px] mx-2 sm:mx-auto">
        ...
        <div className="space-y-3 sm:space-y-4">
          ...
          <div className="space-y-2">
            ...
            <div className="flex items-center gap-2">
              <Input value={apiKey} readOnly className="font-mono text-xs sm:text-sm bg-muted pr-20" />
        ...
        <DialogFooter className="flex flex-col-reverse sm:flex-row gap-2 pt-3 sm:pt-4">
          <Button variant="outline" onClick={downloadApiKey} size="sm" className="w-full sm:w-auto text-xs sm:text-sm">
            <Download className="w-3 h-3 sm:w-4 sm:h-4 mr-2" />
            Download JSON
          </Button>
          <Button onClick={() => onOpenChange(false)} size="sm" className="w-full sm:w-auto text-xs sm:text-sm">
            I've Copied the Key
          </Button>
        </DialogFooter>
      </DialogContent>
...
const CreateApiKeyDialog: React.FC<CreateApiKeyDialogProps> = ({
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      onSuccess((response.data as any)?.key || "sk_live_••••••••••••••••••••••••••••••••", formData.name);
      onOpenChange(false);
      setFormData({
        name: "",
        permissions: [],
  ...
  return <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="w-full rounded-md max-w-[360px] md:max-w-md sm:max-w-lg  mx-0  max-h-[90vh] overflow-y-auto">
        <DialogHeader className="pb-3 sm:pb-4">
        ...
        <form onSubmit={handleSubmit} className="space-y-3 sm:space-y-4">
          <div className="space-y-2">
            ...
            <Input id="name" placeholder="Enter API key name" value={formData.name} onChange={e => setFormData(prev => ({
            ...prev,
            name: e.target.value
          ...
          <div className="space-y-2 sm:space-y-3">
            ...
            <div className="space-y-2 sm:space-y-3">
              {permissions.map(permission => <div key={permission.id} className="flex items-start space-x-2 sm:space-x-3">
                  <Checkbox id={permission.id} checked={formData.permissions.includes(permission.id)} onCheckedChange={checked => handlePermissionChange(permission.id, checked as boolean)} />
                  <div className="grid gap-1 leading-none min-w-0 flex-1">
                    <Label htmlFor={permission.id} className="text-xs sm:text-sm font-medium">
                      {permission.label}
                    </Label>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 sm:gap-4">
            <div className="space-y-2">
              ...
              <Input id="rateLimit" type="number" min="1" value={formData.rateLimit} onChange={e => setFormData(prev => ({
              ...prev,
            ...
            <div className="space-y-2">
              ...
              <Input id="expiresAt" type="datetime-local" value={formData.expiresAt || ""} onChange={e => setFormData(prev => ({
              ...prev,
          ...
          <DialogFooter className="flex flex-col-reverse sm:flex-row gap-2 pt-3 sm:pt-4">
            <Button type="button" variant="outline" onClick={() => onOpenChange(false)} size="sm" className="w-full sm:w-auto text-xs sm:text-sm">
              Cancel
            </Button>
            <Button type="submit" disabled={isLoading} size="sm" className="w-full sm:w-auto text-xs sm:text-sm">
...
const ApiKeyDetailDialog: React.FC<ApiKeyDetailDialogProps> = ({
  ...
  const handleToggleStatus = async () => {
    ...
    try {
      ...
      toast.success(`API key ${apiKey.isActive ? "disabled" : "enabled"} successfully`);
      onUpdate();
    } catch (error) {
      ...
      <DialogContent className="w-full max-w-xs sm:max-w-md lg:max-w-[600px] mx-2 sm:mx-auto max-h-[90vh] overflow-y-auto">
        <DialogHeader className="pb-3 sm:pb-4">
          <DialogTitle className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2">
            ...
            <div className="flex items-center gap-2 flex-shrink-0">
              ...
              {apiKey.expiresAt && new Date(apiKey.expiresAt) < new Date() && <Badge variant="destructive" className="text-xs">
                  Expired
                </Badge>}
            </div>
          </DialogTitle>
        ...
        <div className="space-y-4 sm:space-y-6">
          ...
          <div className="space-y-2">
            ...
            <div className="flex items-center gap-2">
              <Input type={showKey ? "text" : "password"} value={apiKey.key || "sk_live_••••••••••••••••••••••••••••••••"} readOnly className="font-mono text-xs sm:text-sm flex-1" />
              <Button variant="outline" size="sm" onClick={() => setShowKey(!showKey)} className="flex-shrink-0">
                {showKey ? <EyeOff className="w-3 h-3 sm:w-4 sm:h-4" /> : <Eye className="w-3 h-3 sm:w-4 sm:h-4" />}
              </Button>
              <Button variant="outline" size="sm" onClick={() => copyToClipboard(apiKey.key || "")} className="flex-shrink-0">
                <Copy className="w-3 h-3 sm:w-4 sm:h-4" />
              </Button>
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 sm:gap-4">
            ...
            {apiKey.expiresAt && <div className="col-span-1 sm:col-span-2">
                <Label className="text-xs text-muted-foreground">
                  Expires At
                </Label>
                <div className="text-base sm:text-lg font-medium">
        ...
        <DialogFooter className="flex flex-col sm:flex-row gap-2 pt-3 sm:pt-4">
          <Button variant="outline" onClick={handleToggleStatus} disabled={isLoading} size="sm" className="w-full sm:w-auto text-xs sm:text-sm">
            {apiKey.isActive ? "Disable" : "Enable"}
          </Button>

          <Button variant="outline" onClick={handleDelete} disabled={isLoading} className="text-destructive hover:text-destructive w-full sm:w-auto text-xs sm:text-sm" size="sm">
...
const ApiKeyTable: React.FC<ApiKeyTableProps> = ({
  ...
  return <>
        ...
        <CardContent className="p-0">
          <div className="overflow-x-auto overflow-y-auto">
            <Table>
              ...
              <TableBody>
                {apiKeys.map(apiKey => <TableRow key={apiKey.id} className="cursor-pointer hover:bg-muted/50" onClick={() => handleRowClick(apiKey)}>
                    ...
                    <TableCell className="hidden sm:table-cell">
                      <div className="flex flex-wrap gap-1">
                        {apiKey.permissions.slice(0, 2).map(permission => <Badge key={permission} variant="outline" className="text-xs">
                            {permission}
                          </Badge>)}
                        {apiKey.permissions.length > 2 && <Badge variant="outline" className="text-xs">
      ...
      <ApiKeyDetailDialog open={!!selectedApiKey} onOpenChange={open => !open && setSelectedApiKey(null)} apiKey={selectedApiKey} onUpdate={onUpdate} />
    </>;
};
const ApiManagementPage: React.FC = () => {
  ...
  return <MainLayout>
      <div className=" mx-auto  sm:p-0 space-y-4 sm:space-y-6">
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 sm:gap-4">
          <div>
            <h1 className="text-xl sm:text-xl lg:text-2xl font-bold tracking-tight">
              API Management
            </h1>
        ...
        <Tabs defaultValue="keys" className="space-y-4 sm:space-y-6">
          <TabsList className="grid w-full grid-cols-2 sm:grid-cols-4 h-auto">
            <TabsTrigger value="keys" className="flex items-center gap-1 sm:gap-2 text-xs sm:text-sm py-2">
              <Key className="h-3 w-3 sm:h-4 sm:w-4" />
              <span className="hidden xs:inline">API Keys</span>
              <span className="xs:hidden">Keys</span>
            </TabsTrigger>
            <TabsTrigger value="docs" className="flex items-center gap-1 sm:gap-2 text-xs sm:text-sm py-2">
              <BookOpen className="h-3 w-3 sm:h-4 sm:w-4" />
              <span className="hidden xs:inline">Documentation</span>
              <span className="xs:hidden">Docs</span>
            </TabsTrigger>
            <TabsTrigger value="monitoring" className="flex items-center gap-1 sm:gap-2 text-xs sm:text-sm py-2">
              <BarChart3 className="h-3 w-3 sm:h-4 sm:w-4" />
              <span className="hidden sm:inline">Monitoring</span>
              <span className="sm:hidden">Monitor</span>
            </TabsTrigger>
            <TabsTrigger value="testing" className="flex items-center gap-1 sm:gap-2 text-xs sm:text-sm py-2">
          ...
          <TabsContent value="keys" className="space-y-4 sm:space-y-6">
            ...
            {isLoading ? <div className="grid gap-4">
              ...
              </div> : <ApiKeyTable apiKeys={apiKeys} isLoading={isLoading} onUpdate={fetchApiKeys} />}
          </TabsContent>
        ...
        <CreateApiKeyDialog open={createDialogOpen} onOpenChange={setCreateDialogOpen} onSuccess={(apiKey, keyName) => {
        setCreatedApiKey({
          key: apiKey,
          name: keyName
        });
        ...
        <ApiKeyCreatedDialog open={!!createdApiKey} onOpenChange={() => setCreatedApiKey(null)} apiKey={createdApiKey?.key || ""} keyName={createdApiKey?.name || ""} />
      </div>
    </MainLayout>;
};
