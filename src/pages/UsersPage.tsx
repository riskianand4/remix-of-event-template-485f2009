...
import { Search, Download, Upload, Users, UserCheck, UserX, Shield, Activity, AlertTriangle, Loader2 } from 'lucide-react';
...
export default function UsersPage() {
  ...
  const handleEmailVerificationComplete = () => {
    ...
    // Refresh users list to update verification status
    // The useUserManager hook should automatically refetch or we can trigger a refresh
  ...
  const filteredUsers = users.filter(userData => {
    const matchesSearch = userData.name.toLowerCase().includes(searchTerm.toLowerCase()) || userData.email.toLowerCase().includes(searchTerm.toLowerCase()) || userData.department.toLowerCase().includes(searchTerm.toLowerCase());
  ...
  return <ErrorBoundary>
      <MainLayout>
      <div className="space-y-4 sm:space-y-6  sm:p-0 pb-16 sm:pb-6">
         {/* Header */}
        <motion.div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3 sm:gap-4" initial={{
          ...
          <div className="min-w-0 flex-1">
            <h1 className="text-xl sm:text-2xl lg:text-2xl font-bold text-foreground truncate">Manajemen User</h1>
        ...
        {/* Overview Cards */}
        <motion.div className="grid grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-4 lg:gap-6" initial={{
          opacity: 0,
          y: 20
        }} animate={{
          opacity: 1,
        ...
        <motion.div className="flex flex-col gap-3 sm:gap-4" initial={{
          ...
          <div className="relative flex-1">
            ...
            <Input placeholder="Cari berdasarkan nama, email, atau department..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} className="pl-8 sm:pl-10 bg-muted/50 text-sm" />
          </div>
          <div className="grid grid-cols-1 xs:grid-cols-2 lg:grid-cols-4 gap-3">
            ...
            <Select value={selectedDepartment} onValueChange={setSelectedDepartment}>
              ...
              <SelectContent className="bg-background border shadow-md z-50">
                ...
                {departments.map((dept, index) => <SelectItem key={index} value={dept || 'unknown'}>{dept || 'Unknown Department'}</SelectItem>)}
              </SelectContent>
            </Select>
          </div>
        </motion.div>
        ...
        <motion.div initial={{
          ...
          <Tabs defaultValue="users" className="space-y-3 sm:space-y-4">
            ...
            <TabsContent value="users" className="space-y-3 sm:space-y-4">
              {/* Bulk Actions */}
              <BulkUserActions selectedUsers={selectedUsers} onClearSelection={() => setSelectedUsers([])} onBulkStatusUpdate={handleBulkStatusUpdate} onBulkRoleUpdate={handleBulkRoleUpdate} onBulkDelete={handleBulkDelete} onExportUsers={handleExportUsers} onImportUsers={handleImportUsers} />
              ...
              <Card>
                <CardHeader className="pb-3 sm:pb-6">
                  ...
                  <CardDescription className="text-xs sm:text-sm">
                    ...
                    {filteredUsers.length !== totalUsers && <span className="ml-2 text-primary">
                ...
                <CardContent className="p-0 sm:p-6 sm:pt-0">
                  <div className="overflow-x-auto">
                    <UserTable users={filteredUsers} isLoading={loading} onEdit={handleEditUser} onView={handleViewUser} onDelete={handleDeleteUser} onToggleStatus={handleToggleStatus} onViewActivity={handleViewActivity} onVerifyEmail={handleVerifyEmail} />
                  </div>
                </CardContent>
              </Card>
            </TabsContent>
        ...
        <Dialog open={deleteDialogOpen} onOpenChange={setDeleteDialogOpen}>
          <DialogContent className="bg-background rounded-md max-w-[360px] md:max-w-md sm:max-w-lg  mx-0 ">
            <DialogHeader>
        ...
        {/* Edit User Dialog */}
        <EditUserDialog isOpen={editDialogOpen} onClose={() => setEditDialogOpen(false)} user={userToEdit} departments={departments} onUpdate={updateUser} isLoading={loading} />

        {/* View User Dialog */}
        <UserDetailDialog isOpen={viewDialogOpen} onClose={() => setViewDialogOpen(false)} user={userToView} activities={activities} isLoadingActivities={loading} onFetchActivities={fetchUserActivity} />

        {/* Email Verification Dialog */}
        {userToVerify && <EmailVerificationDialog open={verifyEmailDialogOpen} onOpenChange={setVerifyEmailDialogOpen} email={userToVerify.email} type="user_creation" userName={userToVerify.name} onVerificationComplete={handleEmailVerificationComplete} />}
      </div>
      </MainLayout>
    </ErrorBoundary>;
}