...
export const getStockMovementFlow = async (params: { timeFilter?: string; dateRange?: any } = {}): Promise<StockMovementFlow[]> => {
  try {
    ...
    if (response.success && response.data) {
      ...
      console.log('Stock movements from API:', stockMovements);
      
      // Group movements by date
      const groupedByDate = stockMovements.reduce((acc: any, movement: any) => {
        const date = movement._id.date;
        if (!acc[date]) {
          acc[date] = { date, stockIn: 0, stockOut: 0, adjustments: 0, transfers: 0 };
        }
        
        if (movement._id.type === 'in') {
          acc[date].stockIn += movement.quantity || 0;
        } else if (movement._id.type === 'out') {
          acc[date].stockOut += movement.quantity || 0;
        } else if (movement._id.type === 'adjustment') {
          acc[date].adjustments += movement.quantity || 0;
        } else if (movement._id.type === 'transfer') {
          acc[date].transfers += movement.quantity || 0;
        }
      ...
      const result = Object.values(groupedByDate).map((item: any) => ({
        ...item,
        netFlow: item.stockIn - item.stockOut,
        formattedDate: new Date(item.date).toLocaleDateString('id-ID')
      }));
      
      console.log('Processed movement data:', result);
      return result;
    }
    return [];
  } catch (error) {
