...
export const emailVerificationApi = {
  ...
  async verifyEmail(email: string, code: string, type: string): Promise<EmailVerificationResponse> {
    try {
      const response = await fetch(`${ENV.API_BASE_URL}/api/email-verification/verify`, {
        method: 'POST',
        headers: {
  ...
  async resendCode(email: string, type: string): Promise<EmailVerificationResponse> {
    try {
      const response = await fetch(`${ENV.API_BASE_URL}/api/email-verification/resend`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          ...(localStorage.getItem('auth-token') ? { Authorization: `Bearer ${localStorage.getItem('auth-token')}` } : {})
  ...
  async requestEmailChange(newEmail: string): Promise<EmailVerificationResponse> {
    try {
      const response = await fetch(`${ENV.API_BASE_URL}/api/email-verification/request-email-change`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
  ...
  async requestPasswordChange(): Promise<EmailVerificationResponse> {
    try {
      const response = await fetch(`${ENV.API_BASE_URL}/api/email-verification/request-password-change`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          Authorization: `Bearer ${localStorage.getItem('auth-token')}`
