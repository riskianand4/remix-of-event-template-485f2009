...
export class InventoryApiService {
    ...
    try {
      ...
      if (response && response.success && response.data && Array.isArray(response.data)) {
        ...
        const products = (response.data as any[]).map((product: any) => ({
          ...
          // Removed price field - no longer available
          ...
          tags: product.tags || [],
          // Removed costPrice and profitMargin fields - no longer available
  ...
  async createProduct(data: any) {
    ...
    // Remove price-related fields that no longer exist in backend
    delete createData.price;
    delete createData.costPrice;
    delete createData.profitMargin;
  ...
  async updateProduct(id: string, data: any) {
    ...
    delete updateData.profitMargin;
  ...
  // Report methods to match the backend reports API
  async getInventoryReport(params?: any) {
    const queryString = params ? '?' + new URLSearchParams(params).toString() : '';
    const response = await apiClient.get(`/api/reports/inventory${queryString}`);
    return response;
  }

  async getStockMovementsReport(params?: any) {
    const queryString = params ? '?' + new URLSearchParams(params).toString() : '';
    const response = await apiClient.get(`/api/reports/stock-movements${queryString}`);
    return response;
  }

  async getLowStockReport(params?: any) {
    const queryString = params ? '?' + new URLSearchParams(params).toString() : '';
    const response = await apiClient.get(`/api/reports/low-stock${queryString}`);
    return response;
  }

  async getAssetsReport(params?: any) {
    const queryString = params ? '?' + new URLSearchParams(params).toString() : '';
    const response = await apiClient.get(`/api/reports/assets${queryString}`);
    return response;
  }

  async getAbcAnalysisReport(params?: any) {
    const queryString = params ? '?' + new URLSearchParams(params).toString() : '';
    const response = await apiClient.get(`/api/reports/abc-analysis${queryString}`);
    return response;
  }
