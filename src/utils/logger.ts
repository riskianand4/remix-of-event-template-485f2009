...
class Logger {
  ...
  private addLog(level: LogLevel, message: string, data?: any, component?: string): void {
    ...
    if (this.shouldLog(level)) {
      ...
      switch (level) {
        case 'debug':
          if (this.isDevelopment) originalConsole.debug(`🔍 ${prefix}`, message, data);
          break;
        case 'info':
          if (this.isDevelopment) originalConsole.info(`ℹ️ ${prefix}`, message, data);
          break;
        case 'warn':
          originalConsole.warn(`⚠️ ${prefix}`, message, data);
          break;
        case 'error':
  ...
  private async reportError(message: string, data?: any, component?: string): Promise<void> {
    ...
    if (typeof window !== 'undefined' && 'navigator' in window && 'sendBeacon' in navigator) {
      try {
        ...
        originalConsole.error('Failed to report error:', e);
      }
    }
  }
